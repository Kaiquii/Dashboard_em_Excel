...
INPUT = PROJECT_ROOT / "inputs" / DASHBOARD_ID / "CQ Matéria Prima - Fornecedores 2026.xlsm"
WORK_DIR = PROJECT_ROOT / ".tmp" / DASHBOARD_ID
OUTPUT = WORK_DIR / "CQ Matéria Prima - Fornecedores 2026 - Dashboard-openpyxl.xlsm"

DEFAULT_YEAR = 2026
ROWS_PER_MATERIAL = 500
HELPER_FIRST_ROW = 2
EXCLUDED = {"dashboard", "dashboard2", "menu", "padrao", "planilha1"}
...
def find_header(ws):
    ...
    for row in range(1, min(ws.max_row, 12) + 1):
        ...
        score = sum(token in text for token in ("aprov", "fornecedor", "receb", "fabric", "lote", "quantidade", "validade"))
        if score > best_score:
            best_score = score
            best_row = row
    if best_score < 3:
...
def write_dashboard_dynamic(wb, source_sheets, records):
    ...
    merge(ws, "B3:C3", "Fornecedores", COLORS["nav"], Font(name="Segoe UI", color="B8C4D6", size=9), Alignment(horizontal="left", vertical="center"))
    merge(ws, "B6:C7", f'=COUNTIFS($F$67:$F${material_last_row},">0")', COLORS["nav2"], Font(name="Segoe UI", color="FFFFFF", bold=True, size=22), Alignment(horizontal="center", vertical="center"), COLORS["nav2"])
    ws["B6"].number_format = "0"
    merge(ws, "B8:C8", "materias ativas", COLORS["nav2"], Font(name="Segoe UI", color="B8C4D6", bold=True, size=9), Alignment(horizontal="center", vertical="center"), COLORS["nav2"])
    ...
    card(ws, "U5:W9", "MATERIAS ATIVAS", f'=COUNTIFS($F$67:$F${material_last_row},">0")', COLORS["purple"], "0")
    ...
    for source in source_sheets:
        ...
        for offset in range(ROWS_PER_MATERIAL):
            ...
            ws.cell(current_row, 33).value = f'=IF({material_cell}="",0,--ISNUMBER(SEARCH("aprovado",{status_cell})))'
