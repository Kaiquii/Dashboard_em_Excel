...
INPUT = PROJECT_ROOT / "inputs" / DASHBOARD_ID / "CQ Produto Acabado - Procyfloc 2026.xlsm"
WORK_DIR = PROJECT_ROOT / ".tmp" / DASHBOARD_ID
OUTPUT = WORK_DIR / "CQ Produto Acabado - Procyfloc 2026 - Dashboard-openpyxl.xlsm"
...
def product_name(sheet_name):
    return re.sub(r"^procyflo(?:c|f)?\s+", "", sheet_name, flags=re.I).strip() or sheet_name
...
def build():
    ...
    for row in range(16, 26):
        ...
        ws.cell(row, 17).value = f'=IFERROR(IF(LARGE($AO$67:$AO$86,P{row})=0,"",INDEX($E$67:$E$86,MATCH(LARGE($AO$67:$AO$86,P{row}),$AO$67:$AO$86,0))),"")'
        ws.cell(row, 18).value = f'=IF(Q{row}="","",INDEX($G$67:$G$86,MATCH(Q{row},$E$67:$E$86,0)))'
    ...
    status_items = [
        ...
        ("Em andamento", '=COUNTIFS($AJ$2:$AJ$12001,1,$AG$2:$AG$12001,$W$3,$AE$2:$AE$12001,"<>",$AE$2:$AE$12001,"<>*aprov*",$AE$2:$AE$12001,"<>*reprov*")'),
        ("Reprovado", '=COUNTIFS($AJ$2:$AJ$12001,1,$AG$2:$AG$12001,$W$3,$AE$2:$AE$12001,"*reprov*")'),
    ]
    ...
    for idx, source in enumerate(source_sheets, start=67):
        ...
        ws.cell(idx, 41).value = f'=IF(G{idx}>0,G{idx}+ROW()/1000000,0)'
    ...
    for source in source_sheets:
        ...
        for offset in range(ROWS_PER_PRODUCT):
            ...
            ws.cell(current_row, 32).value = f'=IF({product_cell}="",0,--AND(ISNUMBER(SEARCH("aprov",{status_cell})),NOT(ISNUMBER(SEARCH("reprov",{status_cell})))))'
    ...
    ws.cell(66, 41).value = "Ordenacao produto"
    for col in range(24, 42):
