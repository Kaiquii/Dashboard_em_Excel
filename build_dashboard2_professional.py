...
OUTPUT = Path("CQ Produto Acabado - Procytrat 2026 - Dashboard2-profissional-ajustada.xlsm")
...
def find_header(ws):
    ...
    for row in range(1, max_scan_row + 1):
        ...
        score = sum(token in text for token in ("data", "recebimento", "lote", "quantidade", "status", "aprovacao"))
        if score > best_score:
            best_row = row
            best_score = score
            best_values = values
...
def build():
    ...
    card(ws, "E5:H9", "TOTAL DE LOTES", "=SUM($AJ$2:$AJ$12001)", COLORS["blue"], "0")
    card(ws, "I5:L9", "QUANTIDADE TOTAL", "=SUMIFS($AD$2:$AD$12001,$AJ$2:$AJ$12001,1)", COLORS["teal"], '#,##0 "kg"')
    card(ws, "M5:P9", "LOTES APROVADOS", "=SUMIFS($AF$2:$AF$12001,$AJ$2:$AJ$12001,1)", COLORS["green"], "0")
    card(
        ws,
        "Q5:T9",
        "TAXA DE APROVA\u00c7\u00c3O",
        '=IFERROR(SUMIFS($AF$2:$AF$12001,$AJ$2:$AJ$12001,1)/SUM($AJ$2:$AJ$12001),0)',
        COLORS["amber"],
        "0.0%",
    )
    card(ws, "U5:W9", "PRODUTOS ATIVOS", '=COUNTIF($F$67:$F$86,">0")', COLORS["purple"], "0")
    ...
    panel(ws, "S47:W61", "Indicadores r\u00e1pidos")
    ...
    table_style(ws, 50, 13, 54, 15, COLORS["green"])
    status_items = [
        ("Aprovado", '=SUMIFS($AF$2:$AF$12001,$AJ$2:$AJ$12001,1)'),
        ("Em andamento", '=SUMIFS($AJ$2:$AJ$12001,$AE$2:$AE$12001,"*andamento*")'),
        ("Reprovado", '=SUMIFS($AJ$2:$AJ$12001,$AE$2:$AE$12001,"*reprovado*")'),
        ("Simulado", '=SUMIFS($AJ$2:$AJ$12001,$AK$2:$AK$12001,1)'),
    ]
    ...
    for row, color in zip(range(51, 55), [COLORS["green"], COLORS["amber"], COLORS["rose"], COLORS["purple"]]):
    ...
    insight_blocks = [
        ("Produto l\u00edder", '=IFERROR(Q16,"")', '=IFERROR(R16,0)', '#,##0 "kg"'),
        ("M\u00eas mais forte", '=IFERROR(INDEX($P$33:$P$44,MATCH(MAX($R$33:$R$44),$R$33:$R$44,0)),"")', '=IFERROR(MAX($R$33:$R$44),0)', '#,##0 "kg"'),
        ("Pend\u00eancias", "Em andamento + reprovado", '=SUM(N52:N53)', "0"),
    ]
    insight_row = 49
    for title, label_formula, value_formula, fmt in insight_blocks:
        merge(
            ws,
            f"S{insight_row}:W{insight_row}",
            title,
            COLORS["surface2"],
            Font(name="Segoe UI", color=COLORS["muted"], bold=True, size=9),
            Alignment(horizontal="left", vertical="center", indent=1),
        ...
        merge(
            ws,
            f"S{insight_row + 1}:U{insight_row + 1}",
            label_formula,
            COLORS["surface"],
            Font(name="Segoe UI", color=COLORS["ink"], bold=True, size=10),
            Alignment(horizontal="left", vertical="center", indent=1),
            COLORS["line"],
        )
        merge(
            ws,
            f"V{insight_row + 1}:W{insight_row + 1}",
            value_formula,
            COLORS["surface"],
            Font(name="Segoe UI", color=COLORS["blue"], bold=True, size=10),
            Alignment(horizontal="right", vertical="center"),
            COLORS["line"],
        )
        ws[f"V{insight_row + 1}"].number_format = fmt
        insight_row += 4
    ...
    for idx, source in enumerate(source_sheets, start=67):
        ...
        ws.cell(idx, 6).value = f'=SUMIFS($AJ$2:$AJ$12001,$AA$2:$AA$12001,E{idx})'
        ws.cell(idx, 7).value = f'=SUMIFS($AD$2:$AD$12001,$AJ$2:$AJ$12001,1,$AA$2:$AA$12001,E{idx})'
        ws.cell(idx, 8).value = f'=SUMIFS($AF$2:$AF$12001,$AJ$2:$AJ$12001,1,$AA$2:$AA$12001,E{idx})'
        ws.cell(idx, 9).value = f'=IFERROR(H{idx}/F{idx},0)'
        ws.cell(idx, 10).value = f'=IF(G{idx}>0,RANK(G{idx},$G$67:$G$86,0),"")'
        ws.cell(idx, 11).value = (
            f'=IF(F{idx}=0,"",IF(SUMIFS($AK$2:$AK$12001,$AA$2:$AA$12001,E{idx})>0,'
            f'"Simulado",IF(I{idx}>=0.95,"Excelente",IF(I{idx}>=0.8,"Aten\u00e7\u00e3o","Cr\u00edtico"))))'
        )
    ...
    for source in source_sheets:
        ...
        simulated_products.append(source["label"])
        sim_date = f"DATE(2026,{((len(simulated_products) - 1) % 12) + 1},15)"
        sim_qty = 500 + len(simulated_products) * 125
        ws.cell(current_row, 27).value = f'=IF(SUM($AJ${block_start}:$AJ${current_row - 1})=0,"{label}","")'
        ws.cell(current_row, 28).value = f'=IF(AA{current_row}="","",{sim_date})'
        ws.cell(current_row, 29).value = f'=IF(AA{current_row}="","","SIM-{len(simulated_products):03d}")'
        ws.cell(current_row, 30).value = f'=IF(AA{current_row}="",0,{sim_qty})'
        ws.cell(current_row, 31).value = f'=IF(AA{current_row}="","","Simulado")'
        ws.cell(current_row, 32).value = 0
        ws.cell(current_row, 33).value = f'=IF(AB{current_row}="","",YEAR(AB{current_row}))'
        ws.cell(current_row, 34).value = f'=IF(AB{current_row}="","",MONTH(AB{current_row}))'
        ws.cell(current_row, 35).value = f'=IF(AB{current_row}="",0,AB{current_row}+ROW()/1000000)'
        ws.cell(current_row, 36).value = f'=--(AA{current_row}<>"")'
        ws.cell(current_row, 37).value = f'=--(AA{current_row}<>"")'
        current_row += 1

    for idx, year in enumerate(range(2022, 2031), start=2):
        ws.cell(idx, 39).value = year
    validation = DataValidation(type="list", formula1="$AM$2:$AM$10", allow_blank=False)
    ...
    status_chart.add_data(Reference(ws, min_col=14, min_row=50, max_row=54), titles_from_data=True)
    status_chart.set_categories(Reference(ws, min_col=13, min_row=51, max_row=54))
